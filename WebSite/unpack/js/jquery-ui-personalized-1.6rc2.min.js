    ...
    $.ui.mouse, {
        ...
        _init: function() {
            if (this.options.helper == 'original' && !(/^(?:r|a|f)/).test(this.element.css("position")))
                this.element[0].style.position = 'relative'; (this.options.cssNamespace && this.element.addClass(this.options.cssNamespace + "-draggable")); (this.options.disabled && this.element.addClass('ui-draggable-disabled'));
            this._mouseInit();
        },
        _mouseCapture: function(e) {
            var o = this.options;
...
(function($) {
    ...
    $.ui.mouse, {
            ...
            if (!o.placeholder || o.placeholder.constructor == String) {
                ...
                o.placeholder = {
                    ...
                    update: function(container, p) {
                        ...
                        if (!p.height()) {//绘制虚线允许拖拽框的高度
                            p.height(self.currentItem.innerHeight() - parseInt(self.currentItem.css('paddingTop') || 0, 10) - parseInt(self.currentItem.css('paddingBottom') || 0, 10));

                        };
                        if (!p.width()) {//绘制虚线允许拖拽框的宽度
        ...
        _mouseStart: function(e, overrideHandle, noActivation) {//鼠标开始移动事件
            ...
            originalContainer = this.currentItem[0].parentNode; //被拖放块的原始容器
        ...
        _mouseDrag: function(e) {//鼠标拖动事件

            if (this._originalposition == null)
            { this._originalposition = this._generatePosition(e); }
            this.position = this._generatePosition(e);
            this.positionAbs = this._convertPositionTo("absolute");
        ...
        _rearrange: function(e, i, a, hardRefresh) {
            ...
            $(this.placeholder[0]).html(''); //清除画框里面的提示内容
            containerPosition = this.placeholder[0].parentNode; //获取要拖放到的容器
            //开始
            var currentMoveItem = this.containers[0].currentItem[0]; //获取被拖动的对象
            var currentObj = $(currentMoveItem);
            var containerWid = $(containerPosition).width();
            var containerWidHeight = currentObj.height();

            //            if (currentObj.attr("asswidth").toString().indexOf(containerWid) < 0) {
            //                $.ui._noFinalSort = true;
            //                //              下面这句是在画的框里面显示提示信息的
            //                $(this.placeholder[0]).html("<p style='border:solid 3px yellow; height:100%;line-height:30px; width:100%; background-color:White;font-style:oblique; font-family:@黑体; font-size:large; font-weight:bold; font-variant:inherit;text-indent:2em; text-align:center;'>很抱歉！" + currentObj.find('h3').text() + "不允许拖拽到这个宽度，请尝试将该区块拖到其他宽度！</p>");

            //            }
            //            else {
            //$.ui._noFinalSort = false;
            //}
            if (currentObj.attr("asswidth").toString().indexOf(containerWid) < 0) {
                $.ui._noFinalSort = true;
                if (containerWidHeight < 150) {
                    containerWidHeight = 200;
                }
                //              下面这句是在画的框里面显示提示信息的
                $(this.placeholder[0]).css("height", containerWidHeight);
                $(this.placeholder[0]).removeClass();
                var $height = Number(containerWidHeight) - 100;
                $(this.placeholder[0]).html("<div class=\"ins_in\" style=\"height:" + $height + "px;float:left;\" ><div><i></i><p>很抱歉 <em> " + currentObj.find('h3').text() + "</em> 不允许被拖动到这个宽度，请尝试将区块拖动到其他宽度</p></div></div><div style=\"height:0px;*height:10px;_height:10px;float:left;width:100%;\">&nbsp;</div>");

            }
            else {
                $.ui._noFinalSort = false;
                $(this.placeholder[0]).addClass("widget-placeholder");
            }
        ...
        _mouseStop: function(e, noPropagation) {//鼠标停止事件
            if ($.ui.ddmanager && !this.options.dropBehaviour)
        ...
        _clear: function(e, noPropagation) {//做清除操作
            var $currentItem = this.currentItem;
            var $placeholder = this.placeholder;
            var $CheckWidth = Number(this.placeholder.width()) + 4;
            var $CheckHeight = Number(this.placeholder.height());
            if (!$.ui._noFinalSort) {//判断是否可以执行拖放的结果
                if ($(originalContainer).width() != $CheckWidth) {
                    var $data = "&ty=getonewidget&width=" + $CheckWidth + "&widgetid=" + this.currentItem.attr("widgetid") + "&asswidth=" + this.currentItem.attr("asswidth") + "&newwidgetid=" + this.currentItem.attr("newwidgetid") + "&areaid=" + this.placeholder.parent(".widget-place").attr("areaid") + "&frameid=" + $().AjaxWidget.publicVar.$frameid + "&widgetname=" + this.currentItem.find("h3").text() + "&widgetstyleid=" + this.currentItem.attr("widgetstyleid") + "&skinid=" + $().AjaxWidget.publicVar.$skinid + "&moduleid=" + $().AjaxLoad.publicVar.$moduleid +"&oldwidgetid=" + this.currentItem.attr("oldwidgetid")  + "&typeid=" + $().AjaxLoad.publicVar.$TypeID;
                    var $url = "../VisualizationDesign/Ajax/AjaxWidget.aspx";
                    $.ajax({
                        type: "POST",
                        url: $url,
                        timeout: 20000,
                        ...
                        success: function(data, textStatus) {
                            var $text = $currentItem.find("h3").text();
                            $currentItem.after(data); //数据获取成功后将数据添加到页面中
                            //$currentItem.find("h3").text($text);
                            $currentItem.next().find("h3").text($text);
                            $currentItem.remove();
                            //重新扫描页面中可拖拽的区块
                            $.fn.EasyWidgets();
                            //将新插入的取款中增加编辑、删除等操作
                            $().AjaxWidget.InsWidgetEnd();
                            $().AjaxWidget.publicVar.$SaveCheck = false;
                            _M.BindBeforClose();
                        ...
                        error: function() {
                            //请求出错处理
                            //$($ID).text("服务器忙数据加载失败，请刷新页面重新加载数据...");
                        }
                    });
                } else if (this.placeholder.parent(".widget-place").attr("areaid") != $(originalContainer).attr("areaid")) {
                var $data = "&ty=upareaid&width=" + $CheckWidth + "&widgetid=" + $currentItem.attr("widgetid") + "&asswidth=" + $currentItem.attr("asswidth") + "&newwidgetid=" + $currentItem.attr("newwidgetid") + "&areaid=" + this.placeholder.parent(".widget-place").attr("areaid") + "&frameid=" + $().AjaxWidget.publicVar.$frameid + "&widgetname=" + $currentItem.find("h3").text() + "&widgetstyleid=" + $currentItem.attr("widgetstyleid") + "&skinid=" + $().AjaxWidget.publicVar.$skinid + "&moduleid=" + $().AjaxLoad.publicVar.$moduleid;
                    var $url = "../VisualizationDesign/Ajax/AjaxWidget.aspx";
                    $.ajax({
                        type: "POST",
                        url: $url,
                        ...
                        complete: function(XMLHttpRequest, textStatus) {
                            //$().AjaxWidget.WidgetFrameCheck($this);
                            //HideLoading();
                        },
                        error: function() {
                            //请求出错处理
                ...
                //改回body原有的宽度
                if ($().AjaxWidget.publicVar.$thisWidth != null) {
                    var $width = Number($(document.body).width()) - Number($().AjaxWidget.publicVar.$thisWidth);
                    $(document.body).width($width);

                    if ($CheckHeight > $(originalContainer).height()) {
                        $().AjaxWidget.publicVar.$thisHeight = $CheckHeight + 50;
                    }

                    //改回body原有的高度
                    if (navigator.appName.indexOf("Microsoft") != -1) {
                        $height = Number($(document.body).height()) - Number($().AjaxWidget.publicVar.$thisHeight);
                        $(document.body).height($height);

                    } else if (navigator.appName.indexOf("Netscape") != -1) {
                        $height = Number($(document.documentElement).height()) - Number($().AjaxWidget.publicVar.$thisHeight);
                        $(document.documentElement).height($height);

                    }

                    $().AjaxWidget.publicVar.$thisWidth = null;
                }
    ...
    $.extend($.ui.sortable, {
        ...
        defaults: {
            ...
            scope: "default",
            forcePlaceholderSize: false//false
        }
    });
